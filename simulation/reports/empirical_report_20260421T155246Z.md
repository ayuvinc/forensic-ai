...
- ✓ `iban_uae`: Stripped to: 'iban: [IBAN_REDACTED]'
- ✓ `null_bytes`: Stripped to: ''

### What sanitize_pii MISSES (confirmed gaps)

- None — all tested PII types are stripped.
...
- 🛡 `both_empty`: BLOCKED — [validate_input] blocked: Missing required fields: ['workflow', 'case_id']
...
- 🛡 `empty_findings_list`: BLOCKED — [validate_schema] blocked: 1 validation error for JuniorDraft
findings
...
| E3.2 | PM revision feedback threading | ✓ PASS | 2 | 2 | junior_calls=2, pm_calls=2, feedback_in_context='Please add more regulatory cita |
...
| E3.5 | Resume from checkpoint | ✓ PASS | 0 | 1 | After resume from JUNIOR_DRAFT_COMPLETE: call_log=['pm', 'partner'], junior_re_r |

- **E3.2** (SIM-02): ✓ CONFIRMED — junior_calls=2, pm_calls=2, feedback_in_context='Please add more regulatory citations.' — SIM-02 REF
...
|------|--------|-------|---------|------|---------|
| E5.1-zero_risk | RiskItem | likelihood=0, impact=0 | REJECTED | no | SIM-LOW REFUTED |
...
| E5.5-empty_findings | JuniorDraft | findings=[] | REJECTED | no |  |
| E5.6-likelihood=6 | RiskItem | likelihood=6 | REJECTED | no |  |
| E5.6-impact=6 | RiskItem | impact=6 | REJECTED | no |  |
| E5.6-likelihood=0 | RiskItem | likelihood=0 | REJECTED | no |  |
...
| SIM-02 | frm_risk_register revision limits + feedback threading | CONFIRMED | PM feedback threading: REFUTED | Raised RevisionLimitError after junior_calls=3, pm_calls=2 — |
| SIM-04 | Partner rejection 46.4% — no recovery path | CONFIRMED | Raised PipelineError after partner called 1 time(s) — Partne |
| SIM-05..14 | PII (IBAN, special chars) bypass sanitize_pii | REFUTED | 0 PII types bypass hook |
...
| SIM-schema | Schema validation gaps (RiskItem, FinalDeliverable, EvidenceItem) | REFUTED | 0 gaps confirmed:  |
