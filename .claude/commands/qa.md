...
- Review /qa-run results against criteria you wrote.
...
1. Read tasks/ba-logic.md — map each task to its business rule.
2. Read tasks/ux-specs.md if task touches UI.
3. For each task: write measurable, testable acceptance criteria.
4. Criteria must cover: happy path, error state, empty state, auth enforcement, mobile (if UI).
5. Write criteria to task block in tasks/todo.md.

**Mode B — Post-build (review after qa-run):**
1. Read /qa-run results from channel.md.
2. Review built output against acceptance criteria you wrote.
4. Check mobile at 375px if UI was touched.
5. Check auth: can an unauthenticated user reach this?
6. Verdict: QA_APPROVED or QA_REJECTED with written reason.
