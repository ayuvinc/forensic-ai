...
| R-019 | OPEN | SECURITY | P9-01 project_name → slug is a user-controlled string used as a filesystem folder name — path traversal risk if not sanitized (e.g. "../../../etc/passwd") | High | Mitigation: P9-01a Pydantic validator strips all non-alphanumeric/hyphen chars; blocks "..", "/", "\", null bytes; raises ValueError on empty result. Must be tested explicitly in AC. |
...
| R-NEW-04 | OPEN | ARCH | Investigation report has 13 sections — python-docx footnote support is limited; Exhibit N citations may need inline parenthetical references instead | High | Mitigation: assess python-docx footnote capability before WF-01c build; decide approach at build time |
